...
@interface UMSRegisterViewController ()<IUMSSelectRegionViewControllerDelegate>
...
@implementation UMSRegisterViewController
...
- (void)viewDidLoad {
    ...
    self.nextStepButton.frame = CGRectMake(20, CGRectGetMaxY(self.passwordCode.frame)+55, self.view.frame.size.width-40, self.view.frame.size.width*0.11);
    ...
    UILabel *hadAccount = [[UILabel alloc] init];
    hadAccount.frame = CGRectMake(self.view.frame.size.width/2 - 80, CGRectGetMaxY(self.view.frame) - 100 , 90, 35);
    hadAccount.text = @"已有账号?";
    hadAccount.textAlignment = NSTextAlignmentRight;
    hadAccount.font = [UIFont systemFontOfSize:14];
    [self.view addSubview:hadAccount];
    
    //去注册按钮
    self.buttonOfLogin = [UIButton buttonWithType:UIButtonTypeCustom];
    [self.buttonOfLogin setFrame:CGRectMake(CGRectGetMaxX(hadAccount.frame), CGRectGetMaxY(self.view.frame) - 100, 70, 35)];
    [self.buttonOfLogin setTitle:@"去登录" forState:UIControlStateNormal];
    self.buttonOfLogin.titleLabel.textAlignment = NSTextAlignmentLeft;
    self.buttonOfLogin.contentHorizontalAlignment = UIControlContentHorizontalAlignmentLeft;
    [[self.buttonOfLogin titleLabel] setFont:[UIFont systemFontOfSize:14]];
    [self.buttonOfLogin addTarget:self action:@selector(login:) forControlEvents:UIControlEventTouchUpInside];
    [self.buttonOfLogin setTitleColor:[UIColor colorWithRed:220/255.0 green:63/255.0 blue:59/255.0 alpha:1.0] forState:UIControlStateNormal];
    [self.view addSubview:self.buttonOfLogin];
...
-(void)sendSMSCode:(id)sender
{
    ...
    {
        [UMSAlertView showAlertViewWithTitle:@"发送验证码失败"
                                     message:@"手机号不能为空"
                             leftActionTitle:@"好的"
                            rightActionTitle:nil
                              animationStyle:AlertViewAnimationZoom
...
- (void)nextStepAction:(id)sender
{
        ...
        && self.passwordCode.text.length >= 6)
    {
        ...
        [UMSSDK registerWithPhone:self.phoneNumber.text
                         areaCode:self.areaCodeString
                         password:self.passwordCode.text
                          smsCode:self.smsCode.text
                           result:^(UMSUser *user, NSError *error) {
                               ...
                               {
                                   [UMSAlertView showAlertViewWithTitle:@"注册完成"
                                                                message:@""
                                                        leftActionTitle:@"好的"
                                                       rightActionTitle:nil
                                                         animationStyle:AlertViewAnimationZoom
                                                           selectAction:^(AlertViewActionType actionType) {
                                                               ...
                                                               [self.navigationController pushViewController:sup
                                                                                                    animated:YES];
                                                           }];
                               }
